...
class Scheda {
  ...
  add = ({ day, time, courseCode, section, room }) => {
    const color = this.config.colors[this.history.length % this.config.colors.length];
  ...
  get = id => this.history.filter(course => course.id === id)[0];
  ...
  _textAlignCenter = (width, text) => (width - this.ctx.measureText(text).width) / 2;
